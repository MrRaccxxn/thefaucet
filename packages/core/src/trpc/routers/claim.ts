...
const blockchainCooldownCache = new Map<
  string,
  { canClaim: boolean; cooldownSeconds: number; timestamp: number }
>();
const CACHE_TTL_MS = 60000; // 1 minute cache

// Helper function to invalidate cache for a specific user/chain/asset
function invalidateBlockchainCache(
  chainId: number,
  ...
  console.log(
    `[BLOCKCHAIN CACHE] Invalidated cache for ${assetType} asset: ${walletAddress} on chain ${chainId}`
  );
}

// Helper function to check blockchain cooldown for any asset type with caching
async function checkBlockchainCooldown(
  chainId: number,
  walletAddress: string,
  assetType: "native" | "erc20" | "nft",
  contractAddress?: string
  ...
  if (cached && now - cached.timestamp < CACHE_TTL_MS) {
    console.log(
      `[BLOCKCHAIN CACHE] Using cached result for ${walletAddress} on chain ${chainId}`
    );
    return {
      canClaim: cached.canClaim,
      cooldownSeconds: cached.cooldownSeconds,
    };
  }
  ...
  try {
    ...
    console.log(
      `[BLOCKCHAIN] Querying blockchain for ${assetType} asset: ${walletAddress} on chain ${chainId}`
    );
    ...
    if (assetType === "native") {
      ...
      try {
        ...
        const lastClaimTimeFunction =
          tokenContract.getFunction("lastClaimTime");
        ...
        console.log(
          `[BLOCKCHAIN] ERC20 contract ${contractAddress} doesn't have cooldown methods, allowing claim`
        );
        canClaim = true;
        cooldownSeconds = 0;
      }
      ...
      try {
        ...
        // If contract doesn't have cooldown methods, assume it's allowed
        console.log(
          `[BLOCKCHAIN] NFT contract ${contractAddress} doesn't have cooldown methods, allowing claim`
        );
        canClaim = true;
        cooldownSeconds = 0;
      }
    ...
    console.error(
      `Failed to check blockchain cooldown for ${assetType}:`,
      error
    );
    return { canClaim: true, cooldownSeconds: 0 };
  }
...
async function syncDatabaseWithBlockchain(
  ...
  if (blockchainResult.cooldownSeconds > 0) {
    ...
    const claimTime = new Date(
      Date.now() - 24 * 60 * 60 * 1000 + blockchainResult.cooldownSeconds * 1000
    );
    ...
    try {
      ...
      console.log(
        `[BLOCKCHAIN SYNC] Updated database with blockchain state for ${assetType} on chain ${chainId}`
      );
    } catch (syncError) {
      console.error(
        `[BLOCKCHAIN SYNC] Failed to sync database for ${assetType}:`,
        syncError
      );
    }
  }
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        if (rateLimitResult.allowed) {
          console.log(
            "Database rate limit allowed, checking claims table for recent native claims..."
          );
          ...
          if (recentClaims.length > 0) {
            ...
            const canClaimAt = new Date(
              claimTime.getTime() + 24 * 60 * 60 * 1000
            );
            const now = new Date();
        ...
        if (!chainConfig || !chainConfig.isActive) {
          console.error(
            "[DEBUG] Chain not supported or inactive. ChainId:",
            input.chainId
          );
        ...
        const nativeAssets = await ctx.db
          ...
          .from(assets)
          .where(
            and(eq(assets.type, "native"), eq(assets.chainId, input.chainId))
          )
          .limit(1);
        ...
        const formattedNativeAmount = formatAmountForDB(amount);
  ...
  claimDevToken: claimProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const { getDeploymentAddresses } = await import(
        ...
        // Record the claim in database
        // Format amount to remove unnecessary decimals (e.g., "100" instead of "100.000000000000000000")
        const formattedAmount = formatAmountForDB(amountInTokens);

        console.log("Database insertion values:", {
          userId,
          assetId,
          walletAddress: input.walletAddress,
          amountRaw: amountInTokens,
        ...
        try {
          await ctx.db.insert(claims).values({
            ...
            assetId,
            walletAddress: input.walletAddress,
            amount: formattedAmount, // Store clean amount without trailing zeros
            txHash: claimResult.transactionHash,
            status: "pending",
          });
        } catch (dbError) {
        ...
        const devTokenDeployment = devTokenNetworkName
          ? getDeploymentAddresses(devTokenNetworkName)
          : null;
        invalidateBlockchainCache(
          input.chainId,
          input.walletAddress,
          "erc20",
          devTokenDeployment?.devToken
        );
  ...
  claimERC20: claimProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const amount = process.env.ERC20_TOKEN_AMOUNT || "100";
        ...
        // Record the claim in database
        const formattedERC20Amount = formatAmountForDB(amount);
        await ctx.db.insert(claims).values({
          userId,
          assetId: input.assetId,
          walletAddress: input.walletAddress,
          amount: formattedERC20Amount,
          txHash: claimResult.transactionHash,
          status: "pending",
        });
      ...
      try {
        ...
        for (const assetType of assetTypes) {
          ...
          if (rateLimitResult.allowed && walletAddress) {
            console.log(
              `[RATE LIMITING] Database allows ${assetType} claim for user ${userId}, checking blockchain for sync`
            );
            ...
            const blockchainResult = await checkBlockchainCooldown(
              input.chainId,
              walletAddress,
              assetType,
              contractAddress
            ...
            if (
              !blockchainResult.canClaim &&
              blockchainResult.cooldownSeconds > 0
            ) {
              // Blockchain disagrees with database - blockchain has the truth
              console.log(
                `[BLOCKCHAIN SYNC] Database-blockchain mismatch for ${assetType}. DB: allowed, Blockchain: ${blockchainResult.cooldownSeconds}s remaining`
              );

              const cooldownEnds = new Date(
                Date.now() + blockchainResult.cooldownSeconds * 1000
              );
              finalResult = {
                allowed: false,
                cooldownEnds,
                lastClaimAt: new Date(
                  Date.now() -
                    24 * 60 * 60 * 1000 +
                    blockchainResult.cooldownSeconds * 1000
                ),
              };

              // Sync database with blockchain state to prevent future discrepancies
              await syncDatabaseWithBlockchain(
                ctx.db,
                userId,
                input.chainId,
                assetType,
              ...
              console.log(
                `[RATE LIMITING] Database and blockchain in sync for ${assetType}. User can claim.`
              );
            }
          } else {
            // Database says not allowed - trust database completely, no blockchain call needed
            console.log(
              `[RATE LIMITING] Database denies ${assetType} claim for user ${userId}. Blockchain call skipped (cost optimization).`
            );
          }
          ...
          if (!finalResult.allowed && finalResult.cooldownEnds) {
            ...
            const timeRemainingMs =
              finalResult.cooldownEnds.getTime() - now.getTime();
